...
const Interview = ({ interviewData, onFinish }) => {
  ...
  const [selectedVoice, setSelectedVoicde] = useState(null);
  ...
  useEffect(() => {
    const loadVoices = () => {
      ...
      if (maleVoice) {
        setSelectedVoicde(maleVoice);
        setVoiceGender("male");
        return;
      }

      //fallback: first voice(assume female)
      setSelectedVoicde(voices[0]);
      setVoiceGender("female");
    };
  ...
  const speakText = (text) => {
    return new Promise((resolve) => {
      ...
      utterance.pitch = 1.05; // small warth
      utterance.volume = 1;
  ...
  useEffect(() => {
    ...
    const runIntro = async () => {
      if (isIntroPhase) {
        await speakText(
          `Hi ${userName}, it's great to meet you today. I hope your're feeling confident and ready.`,
        );
        ...
        if (currentIndex === questions.length - 1) {
          await speakText("Alright, this one mightbe a bit more challenging.");
        }
        ...
        <div className="flex-1 flex flex-col p-4 sm:p-6 md:p-8 relative">
          ...
          {!feedback ? (
            ...
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="mt-6 bg-red-600 border border-red-200 p-5 rounded-2xl shadow-sm"
            >
