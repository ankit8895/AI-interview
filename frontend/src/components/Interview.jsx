...
const Interview = () => {
  ...
  useEffect(() => {
    if (!isQuestionReady || !currentQuestion) return;
  ...
  }, [isQuestionReady, currentIndex, currentQuestion]);
  ...
  useEffect(() => {
    ...
    const runIntro = async () => {
      if (isIntroPhase) {
        // null — don't restart mic between intro sentences
        await speakText(
        ...
        // Only after all intro speech is done, move to questions
        setIsIntroPhase(false);
      } else if (currentQuestion) {
        setIsQuestionready(false); // reset before the AI speaks
        ...
        if (currentIndex === questions.length - 1) {
          ...
          // no mic between transition lines
        }
        // onSpeechEnd here — mic starts only after question is fully spoken
  ...
  const handleSubmitAnswer = async () => {
    ...
    if (submitAnswer.fulfilled.match(result)) {
      ...
      await speakText(feedbackText, null);
    }
  ...
  const handleNext = async () => {
    setAnswer("");
    setFeedback("");
    setIsQuestionready(false); // stop old timer immediately

    if (currentIndex + 1 >= questions.length) {
      handleFinishInterview();
      return;
    }
    // null — don't start mic during transition sentence
    await speakText("Alright, let's move to the next question.", null);
    setCurrentIndex((prev) => prev + 1);
    // mic will start after the question is spoken via onSpeechEnd in runIntro
  };
